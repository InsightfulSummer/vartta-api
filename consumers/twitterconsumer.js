...
class TwitterConsumer extends BaseConsumer {
  ...
  routes() {
    this.socket.on('update_channels', (data) => {
      this.updateChannels(data)
    })
    this.socket.on('pause_consuming', (data) => {
      this.pauseConsuming()
    })
    this.socket.on('topics_request', (data) => {
      this.ackTopics(data)
    })
    this.socket.on('initial_data_request', (data) => {
      this.sendInitials(data)
    })
    this.socket.on('update_labeling', (data) => {
      this.updateCustomLabels(data)
    })
  }
  ...
  async sendInitials(data) {
    ...
    const aggregateUsers = await axios
      .get(config.get('bakjs').getAggregateUsers)
      .then((response) => {
    ...
    const aggregateTopics = await axios
      .get(config.get('bakjs').getAggregateTopics)
      .then((response) => {
    ...
    const aggregateKeywords = await axios
      .get(config.get('bakjs').getAggregateKeywords)
      .then((response) => {
        return response.data
      })
      .catch((err) => {
    ...
    const windowedTweets = await axios
      .get(config.get('bakjs').getTweets, {
        params: {
          from: new Date(
            new Date().getTime() - 7 * 24 * 60 * 60 * 1000
          ).getTime(), // Yesterday this time
  ...
  updateCustomLabels(data) {
    axios
      .post(config.get('bakjs').updateLabeling, {
  ...
  async handleTweet(tweet) {
    ...
    if (tweet.extended_tweet) tweet.text = tweet.extended_tweet.full_text
